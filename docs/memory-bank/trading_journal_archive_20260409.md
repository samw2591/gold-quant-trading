...
- **2026-04-09 严重方法论错误 — "伪数据驱动"**：
  - **事件**：分析 4/6-4/8 三笔连续亏损日志后，提出"ADX灰区过滤器"（ADX 20-26 区间要求更高 trend_score）、"min_h1_bars_today"、"渐进冷却 x4"三个优化，未经回测直接写入 backtest engine 并推送
  - **错误 1 — 小样本归纳**：从 3 笔亏损案例中提炼"规律"，声称"盈利大单 ADX 通常>28-30"，实际从未做过 ADX 分桶 vs PnL 统计，属于 cherry-picking
  - **错误 2 — 无视已有结论**：trading journal 明确记录 "SELL ADX>=28：不采纳，效果几乎为零"（2026-04-07 回测），而 ADX 因子 IC 扫描结论也是"无效因子（|IC|<0.005，无统计显著性）"，我却提出 ADX 灰区过滤
  - **错误 3 — 推翻已验证参数**：30 分钟冷却期经过 20 变体交叉测试确认最优（2026-04-05），我提议 x4 渐进冷却（120 分钟）没有任何数据支撑
  - **错误 4 — 先改代码后验证**：直接修改 engine.py 和 runner.py，被用户指出"你都没有测试怎么就直接应用了"后才开始验证
  - **根因**：先有了"要加过滤器"的结论，再去找"证据"支持，用专业术语包装主观臆断

## ⛔ 优化提议纪律 (2026-04-09 制定，永久生效)

**任何策略/参数变更提议必须通过以下检查清单，缺一不可：**

### 1. 提议前 — 数据基础检查
- [ ] **明确标注**：这是"假设"还是"已验证结论"？不允许模糊表述
- [ ] **查阅 trading journal**：该方向是否已有历史回测结论？如果已被否定过，必须说明为什么这次不同
- [ ] **样本量**：支撑该提议的数据有多少笔？< 30 笔的观察只能作为"假设待验证"，不能作为行动依据
- [ ] **是否与已有 IC/因子扫描结论矛盾**？如果矛盾，必须先解释

### 2. 提议时 — 诚实表达
- [ ] 分开陈述"数据事实"和"我的推测"，不混为一谈
- [ ] 不用确定性语气描述未验证的观察（禁止"通常"、"都是"、"一般"等词修饰未统计的现象）
- [ ] 如果是从少量案例中归纳的，必须明说"这是从 N 笔案例中的观察，尚未统计验证"

### 3. 实施前 — 强制回测
- [ ] **先回测，后改代码**：任何过滤器/参数变更必须先有全量回测数据证明有效（Sharpe 提升或同等 Sharpe 下 MaxDD 降低）
- [ ] **带成本测试**：必须包含 $0.30-$0.50 点差的测试结果
- [ ] **不动已验证参数**：除非新回测数据**明确优于**现有参数，否则不改。"可能更好"不够，要"数据证明更好"

### 4. 违反后果
- 违反以上任何一条，该提议自动作废
- 记录到"踩过的坑"，包括完整错误链条，防止重犯
